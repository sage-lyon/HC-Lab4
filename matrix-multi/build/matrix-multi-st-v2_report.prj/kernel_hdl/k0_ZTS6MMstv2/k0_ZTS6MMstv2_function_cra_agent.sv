...
// SystemVerilog created from k0_ZTS6MMstv2_function_cra_agent
...
module k0_ZTS6MMstv2_function_cra_agent (
    ...
    output wire [1855:0] kernel_arguments,
    ...
    reg [31:0] arguments_56_q;
    reg [31:0] arguments_56_buffered_q;
    reg [31:0] arguments_57_q;
    reg [31:0] arguments_57_buffered_q;
    ...
    wire [1855:0] kernel_arg_bit_join_q;
    ...
    wire [127:0] dupName_15_arg_bit_join_x_q;
    ...
    wire [5:0] dupName_42_address_ref_x_q;
    wire [0:0] dupName_42_can_write_x_q;
    ...
    // printf_bit(BITSELECT,251)
    assign printf_bit_b = status_NO_SHIFT_REG_q[3:3];

    // printf_bit_mux(MUX,252)
    ...
    // readdata_valid_stage1_reg(REG,259)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // finish_counter_NO_SHIFT_REG(REG,155)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // can_write(LOGICAL,132)
    assign can_write_q = NO_NAME_q & avs_cra_write;

    // status_select(MUX,285)
    ...
    // cra_output_was_finish_counter_address_stage2_reg(REG,150)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // cra_output_readdata_reg(REG,147)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // avs_cra_readdata(GPOUT,230)
    assign avs_cra_readdata = cra_output_readdata_reg_q;

    // readdata_valid_stage2_reg(REG,260)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // cra_output_readdatavalid_reg(REG,148)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_0(REG,171)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_0(REG,165)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_1(REG,173)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_1(REG,167)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_2(REG,175)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_2(REG,169)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_0(REG,180)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_1(REG,182)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_2(REG,184)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_size_2_bit_join(BITJOIN,179)
    assign global_size_2_bit_join_q = {global_size_reg_upper_0_q, global_size_reg_lower_2_q};

    // global_size_2(GPOUT,238)
    assign global_size_2 = global_size_2_bit_join_q;

    // dupName_42_address_ref_x(CONSTANT,566)
    assign dupName_42_address_ref_x_q = $unsigned(6'b101011);

    // dupName_42_NO_NAME_x(LOGICAL,565)
    assign dupName_42_NO_NAME_x_q = $unsigned(avs_cra_address == dupName_42_address_ref_x_q ? 1'b1 : 1'b0);

    // dupName_42_can_write_x(LOGICAL,567)
    assign dupName_42_can_write_x_q = dupName_42_NO_NAME_x_q & avs_cra_write;

    // dupName_74_mask0_x(LOGICAL,664)
    assign dupName_74_mask0_x_q = bit_enable_top_bar_b & arguments_57_buffered_q;

    // dupName_74_new_data_x(LOGICAL,666)
    assign dupName_74_new_data_x_q = dupName_74_mask0_x_q | dupName_1_mask1_x_q;

    // arguments_57_buffered(REG,113)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            arguments_57_buffered_q <= $unsigned(32'b00000000000000000000000000000000);
        end
        else if (dupName_42_can_write_x_q == 1'b1)
        begin
            arguments_57_buffered_q <= dupName_74_new_data_x_q;
        end
    end

    // arguments_57(REG,112)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            arguments_57_q <= $unsigned(32'b00000000000000000000000000000000);
        end
        else if (start_from_buffered_start_NO_SHIFT_REG_q == 1'b1)
        begin
            arguments_57_q <= arguments_57_buffered_q;
        end
    end

    // dupName_73_mask0_x(LOGICAL,661)
    assign dupName_73_mask0_x_q = bit_enable_bottom_bar_b & arguments_56_buffered_q;

    // dupName_73_new_data_x(LOGICAL,663)
    assign dupName_73_new_data_x_q = dupName_73_mask0_x_q | mask1_q;

    // arguments_56_buffered(REG,111)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            arguments_56_buffered_q <= $unsigned(32'b00000000000000000000000000000000);
        end
        else if (dupName_42_can_write_x_q == 1'b1)
        begin
            arguments_56_buffered_q <= dupName_73_new_data_x_q;
        end
    end

    // arguments_56(REG,110)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            arguments_56_q <= $unsigned(32'b00000000000000000000000000000000);
        end
        else if (start_from_buffered_start_NO_SHIFT_REG_q == 1'b1)
        begin
            arguments_56_q <= arguments_56_buffered_q;
        end
    end

    // dupName_41_address_ref_x(CONSTANT,560)
    assign dupName_41_address_ref_x_q = $unsigned(6'b101010);

    // dupName_41_NO_NAME_x(LOGICAL,559)
    assign dupName_41_NO_NAME_x_q = $unsigned(avs_cra_address == dupName_41_address_ref_x_q ? 1'b1 : 1'b0);

    // dupName_41_can_write_x(LOGICAL,561)
    assign dupName_41_can_write_x_q = dupName_41_NO_NAME_x_q & avs_cra_write;

    // dupName_72_mask0_x(LOGICAL,658)
    assign dupName_72_mask0_x_q = bit_enable_top_bar_b & arguments_55_buffered_q;

    // dupName_72_new_data_x(LOGICAL,660)
    ...
    // dupName_15_arg_bit_join_x(BITJOIN,403)
    assign dupName_15_arg_bit_join_x_q = {arguments_57_q, arguments_56_q, arguments_55_q, arguments_54_q};

    // dupName_40_address_ref_x(CONSTANT,554)
    assign dupName_40_address_ref_x_q = $unsigned(6'b101001);

    // dupName_40_NO_NAME_x(LOGICAL,553)
    assign dupName_40_NO_NAME_x_q = $unsigned(avs_cra_address == dupName_40_address_ref_x_q ? 1'b1 : 1'b0);

    // dupName_40_can_write_x(LOGICAL,555)
    assign dupName_40_can_write_x_q = dupName_40_NO_NAME_x_q & avs_cra_write;

    // dupName_70_mask0_x(LOGICAL,652)
    assign dupName_70_mask0_x_q = bit_enable_top_bar_b & arguments_53_buffered_q;

    // dupName_70_new_data_x(LOGICAL,654)
    ...
    // dupName_14_arg_bit_join_x(BITJOIN,395)
    assign dupName_14_arg_bit_join_x_q = {arguments_53_q, arguments_52_q, arguments_51_q, arguments_50_q};

    // dupName_38_address_ref_x(CONSTANT,542)
    assign dupName_38_address_ref_x_q = $unsigned(6'b100111);

    // dupName_38_NO_NAME_x(LOGICAL,541)
    assign dupName_38_NO_NAME_x_q = $unsigned(avs_cra_address == dupName_38_address_ref_x_q ? 1'b1 : 1'b0);

    // dupName_38_can_write_x(LOGICAL,543)
    assign dupName_38_can_write_x_q = dupName_38_NO_NAME_x_q & avs_cra_write;

    // dupName_66_mask0_x(LOGICAL,640)
    assign dupName_66_mask0_x_q = bit_enable_top_bar_b & arguments_49_buffered_q;

    // dupName_66_new_data_x(LOGICAL,642)
    ...
    // dupName_13_arg_bit_join_x(BITJOIN,387)
    assign dupName_13_arg_bit_join_x_q = {arguments_49_q, arguments_48_q, arguments_47_q, arguments_46_q};

    // dupName_36_address_ref_x(CONSTANT,530)
    assign dupName_36_address_ref_x_q = $unsigned(6'b100101);

    // dupName_36_NO_NAME_x(LOGICAL,529)
    assign dupName_36_NO_NAME_x_q = $unsigned(avs_cra_address == dupName_36_address_ref_x_q ? 1'b1 : 1'b0);

    // dupName_36_can_write_x(LOGICAL,531)
    assign dupName_36_can_write_x_q = dupName_36_NO_NAME_x_q & avs_cra_write;

    // dupName_62_mask0_x(LOGICAL,628)
    assign dupName_62_mask0_x_q = bit_enable_top_bar_b & arguments_45_buffered_q;

    // dupName_62_new_data_x(LOGICAL,630)
    ...
    // dupName_12_arg_bit_join_x(BITJOIN,379)
    assign dupName_12_arg_bit_join_x_q = {arguments_45_q, arguments_44_q};

    // dupName_35_address_ref_x(CONSTANT,524)
    ...
    // workgroup_size_NO_SHIFT_REG(REG,292)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
